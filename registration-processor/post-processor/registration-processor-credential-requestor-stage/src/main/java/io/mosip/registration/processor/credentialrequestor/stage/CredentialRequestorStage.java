...
public class CredentialRequestorStage extends MosipVerticleAPIManager {
	...
	private void callCredIssuer(String regId, String uin, String process) {
		try {
			Map<String, Object> request = buildCredIssuerRequest(regId, uin);
			...
			//For testing
			logCredentialFieldsForTesting(regId, process);
		} catch (Exception e) {
			regProcLogger.error(LoggerFileConstant.SESSIONID.toString(), LoggerFileConstant.REGISTRATIONID.toString(),
	...
	private void logCredentialFieldsForTesting(String regId, String process) {
		try {
			...
			String address = JsonUtil.getJSONValue(
			...
			regProcLogger.info(
					...
					regId,
					"CredentialRequestorStage :: Testing field values = " + fieldMap
			);
		} catch (Exception e) {
			regProcLogger.error(
					LoggerFileConstant.SESSIONID.toString(),
					LoggerFileConstant.REGISTRATIONID.toString(),
					regId,
					"CredentialRequestorStage :: Failed to log test credential fields: "
							+ e.getMessage()
			);
		}
	}

	private Map<String, Object> buildCredIssuerRequest(String regId, String uin) {
		regProcLogger.info(LoggerFileConstant.SESSIONID.toString(), LoggerFileConstant.REGISTRATIONID.toString(), "",
